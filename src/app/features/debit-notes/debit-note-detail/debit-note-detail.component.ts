...
@Component({
  ...
  template: `
    ...
    <div class="no-print action-bar">
      <div class="ab-left">
        <a mat-stroked-button routerLink="/debit-notes" class="back-btn">
          <mat-icon>arrow_back</mat-icon> Back
        </a>
        <span class="ab-title">Debit Note {{ note?.noteNumber }}</span>
        <span class="ab-sub">GST Supplier Adjustment Voucher</span>
      </div>
      <div class="ab-right">
        <button mat-stroked-button (click)="sendEmail()" [disabled]="!note?.supplier?.email" class="email-btn">
          <mat-icon>email</mat-icon> Send Email
        </button>
        <button mat-raised-button color="primary" (click)="print()">
          <mat-icon>print</mat-icon> Print Debit Note
        </button>
      </div>
    </div>
    ...
    <div class="preview-wrap" *ngIf="note">
      <div class="inv-doc">
        ...
        <div class="inv-hdr">
          ...
          <div class="hdr-right">
            <div class="inv-ttl">DEBIT NOTE</div>
        ...
        <div class="info-band">
          <div class="party-col">
            <div class="pb-label">🧾 Supplier Details (To)</div>
            <div class="pb-name">{{ note.supplier?.name }}</div>
            <div class="pb-line">{{ note.supplier?.address }}</div>
            <div class="pb-line" *ngIf="note.supplier?.phone">Ph: {{ note.supplier?.phone }}</div>
            <div class="pb-line" *ngIf="note.supplier?.gstin">GSTIN: {{ note.supplier?.gstin }}</div>
            <div class="pb-line" *ngIf="note.supplier?.state">State: {{ note.supplier?.state?.name }}, Code: {{ note.supplier?.state?.gstCode }}</div>
          </div>

          <div class="meta-col">
            <div class="meta-grid">
              <div class="mf"><span class="mk">Debit Note No.</span><span class="mv">{{ note.noteNumber }}</span></div>
              <div class="mf"><span class="mk">Voucher Date</span><span class="mv">{{ note.noteDate | date:'d-MMM-yyyy' }}</span></div>
              <div class="mf"><span class="mk">Ref Purchase Ref</span><span class="mv">{{ note.purchaseInvoice?.invoiceNumber || '—' }}</span></div>
              <div class="mf"><span class="mk">Purchase Invoice Date</span><span class="mv">{{ note.purchaseInvoice?.invoiceDate ? (note.purchaseInvoice?.invoiceDate | date:'d-MMM-yyyy') : '—' }}</span></div>
              ...
              <tr *ngFor="let item of note.items; let i = index" class="it-row">
                ...
                <td>
                  ...
                  <span class="gst-tag" *ngIf="item.gstPercentage">
                    <ng-container *ngIf="company?.state?.id !== note.supplier?.state?.id && note.supplier?.state; else cgstSgst">
        ...
        <div class="ft-band">
          <div class="ft-bank">
            ...
            <p class="decl">This debit note serves as formal notification of invoice adjustment, sales return, or rate reduction applied to purchase balances.</p>
          </div>
          <div class="ft-decl">
          </div>
          <div class="ft-sig">
        ...
        <!-- GENERATED BY -->
        <div class="cg-foot">Computer Generated Debit Note Adjustment Voucher</div>
  ...
  styles: [`
    ...
    .ft-band { display:grid; grid-template-columns:2fr 125px; gap:0; border-top:1.5px solid #e2e8f0; margin:8px 12px 0; padding-top:8px; }
    .ft-bank, .ft-sig { padding:0 8px; }
    .ft-bank { padding-left:0; }
    ...
    .decl { font-size:9px; color:#64748b; line-height:1.55; margin:0; }
...
export class DebitNoteDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (id) this.debitNoteService.getById(+id).subscribe(data => this.note = data);
  }
  ...
  sendEmail(): void {
    ...
    this.dialog.open(EmailDialogComponent, {
      data: {
        toEmail: this.note.supplier?.email || '',
        subject: `Debit Note ${this.note.noteNumber} from ${this.company?.companyName || 'Us'}`,
        message: `Dear ${this.note.supplier?.name || 'Supplier'},\n\nPlease find attached debit adjustment voucher ${this.note.noteNumber} for our account.\n\nThank you!`,
        documentType: 'DEBIT_NOTE',
        documentId: this.note.id!,
        documentLabel: `Debit Note ${this.note.noteNumber}`
      },
      width: '620px'
    });
  }
  ...
  private buildPrintDoc(): string {
    ...
    const cgst = dn.totalCgst  || 0;
    const sgst = dn.totalSgst  || 0;
    const igst = dn.totalIgst  || 0;
    const showTax = cgst > 0 || igst > 0;
    const isIGST  = (co?.state?.id !== dn.supplier?.state?.id) && !!dn.supplier?.state;

    const itemRows = (dn.items || []).map((item, i) => {
    ...
    const totalQty = (dn.items || []).reduce((s, it) => s + (it.quantity || 0), 0);
    ...
    let hsnFoot = `<td><b>Total</b></td><td class="tr"><b>${N(dn.totalTaxableValue)}</b></td>`;
    ...
    const su = dn.supplier;
    const addrLines = (name: string, addr: string, phone: string, gstin: string, state: string) =>
      `<div class="pan">${name}</div>
       <div class="pal">${addr || ''}</div>
    ...
    const billBlock = `<div class="pb">
      <div class="pbl">SUPPLIER DETAILS (TO)</div>
      ${addrLines(
        su?.name || '',
        su?.address || '',
        su?.phone || '',
        su?.gstin || '',
        su?.state ? `State: ${su.state.name}, Code: ${su.state.gstCode}` : ''
      )}
    </div>`;
    ...
    const metaHTML = `
      ${mf('Debit Note No.', H(dn.noteNumber))}
      ${mf('Date', D(dn.noteDate))}
      ${mf('Ref Purchase Invoice', H(dn.purchaseInvoice?.invoiceNumber))}
      ${mf('Purchase Invoice Date', D(dn.purchaseInvoice?.invoiceDate))}
      ${mf('Adjustment Reason', this.getFriendlyReason(dn.reason))}
      ${mf('Total Adjusted Amount', `₹ ${N(dn.grandTotal)}`)}`;

    return `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="utf-8">
<title>Debit Note ${dn.noteNumber}</title>
...
<style>
  *,*::before,*::after{box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
  ...
  .ft{display:grid;grid-template-columns:2fr 125px;border-top:1.5px solid #e2e8f0;margin:6px 10px 0;padding-top:7px}
  .fb{padding-right:8px}
...
<div class="hdr">
  ...
  <div class="hr">
    <div class="ht">DEBIT NOTE</div>
    <div class="ho">VOUCHER ADJUSTMENT</div>
    <div class="hn">${dn.noteNumber || ''}</div>
    <div class="hd">${D(dn.noteDate)}</div>
    <span class="sc ${(dn.status || '').toLowerCase()}">${dn.status || ''}</span>
  </div>
</div>
  ...
  <table class="it">
    ...
    <tfoot>
      <tr>
        ...
        <td></td><td></td>
        <td class="tr ta-v">&#8377; ${N(dn.grandTotal)}</td>
      </tr>
    </tfoot>
  </table>
</div>
...
<div class="wb">
  <div class="wi">
    ...
    <span class="wt">${this.getAmountInWords(dn.grandTotal || 0)}</span>
  </div>
...
<div class="cg">Computer Generated Debit Note Adjustment Voucher</div>
